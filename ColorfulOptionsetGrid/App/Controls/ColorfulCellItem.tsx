...
export const ColorfulCellItem = function ColorfulCellItem({ currentValue, currentDisplayName, metadataOptions, displayTextType, displayIconType, defaultIcon, className, onChange }: IColorfulCellItemProps): JSX.Element {
    ...
    //alert("0 currentValue = '" + currentValue + "'");
    //alert("0 currentDisplayName = '" + currentDisplayName + "'");
    //alert("0 displayTextType = '" + displayTextType + "'");
    ...
    const iconColor = displayTextType === "BOX" ? "#20262B" : color;
    const renderIcon = displayIconType !== "NONE" ? <Icon className="ColorfulOptionsetGrid colorIcon" style={{ color: iconColor, marginRight: "5px" }} iconName={icon} aria-hidden="true" /> : "";
    const style = {
        "BORDER": {
            borderWidth: "1px",
            borderStyle: "solid",
